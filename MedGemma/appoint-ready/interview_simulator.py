...
def patient_roleplay_instructions(patient_name, condition_name, previous_answers):
    ...
    return f"""
        SYSTEM INSTRUCTION: Always respond in Japanese. Before the interview begins, silently review the optional symptoms and decide which ones you have.
...
def interviewer_roleplay_instructions(patient_name):
    ...
    return f"""
        SYSTEM INSTRUCTION: Always respond in Japanese. Always think silently before responding.
...
The entire report must be written in Japanese.
</output_format>"""
...
The final output MUST be ONLY the full, updated Markdown medical report.
DO NOT include any introductory phrases, explanations, or any text other than the report itself.
**The entire updated report must be written in Japanese.**
</output_format>
