...
const emilyWilliamsDischargeSlides: Slide[] = [
    ...
    { title: "退院計画", points: ["安定した状態で退院[8]。", "経口薬で疼痛は良好にコントロール。", "イブプロフェンとシクロベンザプリンの処方箋を送信。", "1週間後に主治医を再診。再診の注意点を説明済み。"] }
];
...
**Background:** 患者にはCADの既往歴はありません。2時間の6/10の胸圧で来院しました。救急外来でアスピリン324mgを投与されました[4]。その後、痛みは2/10に改善しました[5]。
**Assessment:** 初期の心電図では、虚血性変化のない正常洞調律が示されました[2]。最初の心筋酵素セットでは、トロポニンIが陰性でした[3]。バイタルは安定しています。現在、遠隔測定ユニットで快適に休息しています。
**Recommendation:** 遠隔測定で監視を続行します。プロトコルに従って、14:15と20:15に連続心筋酵素を取得します。通常の食事で問題ありません。
`
    },
    {
...
入院期間中、その後の2回のトロポニン測定も陰性であり、急性心筋梗塞は除外されました[6, 8]。患者は滞在中、無症状で血行動態も安定していました[7, 9]。心臓病科にコンサルトしたところ、外来での核医学ストレステストとリスク層別化のためのスタチン療法開始を推奨されました[11]。
...
**P - Patient Summary:** 安藤京香は、原因不明の敗血症性ショックで救急外来から入院した31歳の女性です[1]。発熱、錯乱、重度の低血圧で来院しました。気道確保のためICUで挿管されました[4]。
**A - Action List:**
- 輸液蘇生を続行します。
- MAP > 65 mmHgを維持するためにノルエピネフリンを調整します[3]。
- 広域抗生物質（バンコマイシン、ゾシン）を続行します[3]。
- 乳酸値や腎機能を含む検査室を監視します。
**S - Situation Awareness & Contingency Planning:** 敗血症の疑いが強い。血液培養は保留中です[2]。血行動態の不安定性の悪化や末端器官の損傷に備えてください。昇圧剤の必要性が高まり続ける場合は、バソプレシンなどの第2の薬剤の追加を検討してください。
**S - Synthesis by Receiver:** 受け手のミラー医師は計画を確認し、同意しました。現在の管理を継続し、培養データを待ちます。
`
    },
    {
      ...
      content: `
### SBAR Handoff
**Situation:** こちらは敗血症性ショックのICU-2の安藤京香、31歳女性です。
**Background:** 昨日、敗血症性ショックで入院し[1]、挿管され、広域抗生物質とノルエピネフリンの投与が開始されました[3, 4]。
**Assessment:** 患者は依然として重篤な状態です。夜間に急性腎障害を発症し、クレアチニンが2.5に上昇しました[6]。昇圧剤と鎮静剤を投与されています[5]。尿量はごくわずかです。治療を絞り込むための血液培養の結果をまだ待っています。
**Recommendation:** 現在の支持療法を継続してください。血液培養の結果を至急フォローアップしてください。腎機能が改善しない場合は、腎臓超音波検査を検討してください。家族に厳しい予後について最新情報を伝えてください。
`
    },
    {
...
**Background:** 患者は敗血症性ショックで入院[1]。血液培養でMSSAが陽性であることが判明[7]。
**Assessment:** 感染症科にコンサルト[8]。今朝、抗生物質をナフシリンにデエスカレーション[9]。変更後、昇圧剤の必要量が減少し始めている[10]。鎮静・換気状態は続いているが、血行動態は改善している。
**Recommendation:** バイタルを監視し、許容範囲内でノルエピネフリンを漸減する。ナフシリンへの反応の兆候を監視する。換気患者の標準的なICUケアを継続する。
`
    }
  ],
  3: [ // 高橋啓介
...
**P - Patient Summary:** 高橋啓介は、ACS除外のため観察ユニットに入院した70歳の男性です[3]。断続的で非労作性の胸圧を訴えて来院しました[1]。
**A - Action List:**
- 17:00に2回目の心筋酵素セットを待ちます。
- 監視を続行します。
- 胸痛にはPRNニトログリセリンが利用可能です[5]。
**S - Situation Awareness & Contingency Planning:** HEARTスコアは3で、低リスクを示しています[3]。最初のトロポニンと心電図は陰性でした[2]。2回目のトロポニンが陰性で、患者が無痛のままであれば、朝に退院の候補となります。痛みが再発したり、酵素が陽性の場合は、入院に切り替えて心臓病科に相談します。
**S - Synthesis by Receiver:** ジョンソン医師は計画を確認しました。夕方に検査室を確認し、再評価します。
`
    },
    {
...
**Background:** 患者は低速MVAでシートベルトを着用した運転手でした[1]。首と背中の上部の痛みを訴えています。
**Assessment:** 頭部と頸椎のCTスキャンは陰性でした[2]。患者は意識清明で、神経学的検査は安定しています。痛みは5/10で、PRNイブプロフェンとシクロベンザプリンで管理されています[3, 4]。四肢の動きは良好です。
**Recommendation:** 神経学的状態の監視を継続します。必要に応じて鎮痛薬を投与し、その効果を記録します。許容範囲内で穏やかな運動を奨励します。
`
    },
    {
...
export const MOCK_RECORDS: { [key: number]: PatientRecord[] } = {
    ...
    3: [ // 高橋啓介, 胸痛の観察
        ...
        { id: 30, citationId: 8, type: RecordType.ProgressNote, timestamp: '2023-10-28 09:30', content: '連続酵素陰性。心電図変化なし。痛みは解消。これは非心臓性胸痛と一致する。患者は外来でのストレステストのため、厳密なプライマリケアと心臓病科のフォローアップで退院可能。' },
    ],
    4: [ // 伊藤さくら, 頸部捻挫を伴うMVA
        ...
        { id: 38, citationId: 8, type: RecordType.ProgressNote, timestamp: '2023-10-20 17:00', content: '退院サマリーメモ: 患者は退院可能。MVAによる頸部捻挫の診断。イブプロフェンとシクロベンザプリンの処方箋を薬局に送信。1週間後に主治医とフォローアップするように指示。再診の注意点を説明済み。' },
    ],
};